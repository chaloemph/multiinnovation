    ...
    if(isset($_POST["do"]) && $_POST["do"] != "" ){
        ...
        switch($do){
            ...
            case 'updatedata_p_ack':
                ...
                $AJY_RATE_P_RANK = $_POST['AJY_RATE_P_RANK'];
                $SALARY_ID = $_POST['SALARY_ID'];
                $EXPERT_MIL_ID = $_POST['EXPERT_MIL_ID'];
                $AJY_RATE_P_NUMBER = $_POST['AJY_RATE_P_NUMBER'];
                $AJY_RATE_P_REMARK = $_POST['AJY_RATE_P_REMARK'];
                $AJY_RATE_P_VERSION = 1;
                ...
                $AJY_RATE_P_NUMBER = ($AJY_RATE_P_NUMBER == '')? 0:$AJY_RATE_P_NUMBER;
                $SALARY_ID = ($SALARY_ID == '')? 0:$SALARY_ID;
                $sql = "INSERT INTO j3_ratepersonal_ajy(AJY_RATE_P_NUM, ROST_CPOS, EXPERT_MIL_ID, AJY_RATE_P_REMARK, AJY_RATE_P_NUMBER, AJY_RATE_P_RANK, SALARY_ID, AJY_ID, AJY_RATE_P_VERSION , ROST_OLD_ID) "
                . "VALUES(NULL, :ROST_CPOS, :EXPERT_MIL_ID, :AJY_RATE_P_REMARK, :AJY_RATE_P_NUMBER, :AJY_RATE_P_RANK, :SALARY_ID, :AJY_ID, :AJY_RATE_P_VERSION , :ROST_OLD_ID)";
                $stmt=$db->prepare($sql);
                $stmt->bindparam(':ROST_CPOS',$ROST_CPOS);
                $stmt->bindparam(':EXPERT_MIL_ID',$EXPERT_MIL_ID);
                $stmt->bindparam(':AJY_RATE_P_REMARK',$AJY_RATE_P_REMARK);
                $stmt->bindparam(':AJY_RATE_P_NUMBER',$AJY_RATE_P_NUMBER);
                $stmt->bindparam(':AJY_RATE_P_RANK',$AJY_RATE_P_RANK);
                $stmt->bindparam(':SALARY_ID',$SALARY_ID);
                $stmt->bindparam(':AJY_ID',$AJY_ID);
                $stmt->bindparam(':AJY_RATE_P_VERSION',$AJY_RATE_P_VERSION);
                $stmt->bindparam(':ROST_OLD_ID',$ROST_OLD_ID);
                if ($stmt->execute()){
                    // echo json_encode('success');
                    $AJY_RATE_P_NUM = $db->lastInsertId();
                }else{
                ...
                $sql_update = "UPDATE `j3_ratepersonal_ajy` SET ROST_ID = ? WHERE AJY_RATE_P_NUM = ?  ";
                $stmt3=$db->prepare($sql_update);
                
                if ($stmt3->execute([$ROST_ID , $AJY_RATE_P_NUM])){
                    echo json_encode('success');
                }else{
