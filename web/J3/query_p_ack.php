    ...
    if(isset($_POST["do"]) && $_POST["do"] != "" ){
        ...
        switch($do){
            case 'modal_edit_p_ack':
                ...
                j1_rank.ROST_RANK,
                j3_rebirth.*,
                j3_nrpt_approve.NRPT_NAME,
                j3_nrpt_approve.NRPT_ACM
                FROM j3_rost_approve 
                LEFT JOIN j1_rank ON(j3_rost_approve.ROST_RANK = j1_rank.ROST_RANK AND j3_rost_approve.ROST_RANKNAME = j1_rank.ROST_RANKNAME) 
                LEFT JOIN j3_rebirth ON(j3_rost_approve.ROST_LAO_MAJ = j3_rebirth.CLAO_NAME_SHORT)
                LEFT JOIN j3_nrpt_approve ON(j3_nrpt_approve.UNIT_CODE = j3_rost_approve.ROST_UNIT)
                WHERE ROST_ID = :ID ";
                $ID = $_POST["rost_id"];
